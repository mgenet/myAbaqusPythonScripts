...
volumes   = [data[1] for key in odb.steps.keys() for data in odb.steps[key].historyRegions[odb.steps[key].historyRegions.keys()[num_history_region]].historyOutputs['CVOL'].data]
...
open(odb_basename+'.fluid_cavity_volumes_and_pressures.dat', 'w').write("\n".join([" ".join([str(pv) for pv in pvs]) for pvs in zip(volumes, pressures)]))
